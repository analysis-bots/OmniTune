...
from tools import mock_ups, constants
...
def change_button_shape(button_index=0, margin_top=0):
    html(f"""
    <script>
...
def main():
    ...
    left_half, right_half = st.columns([9, 11])
    ...
    with right_half:
        ...
        with tab_objects[0]:
            ...
            if st.session_state.got_input:
                tabs2.append("Original Query Output")
    ...
    if 'fairness_objective_getter' not in st.session_state:
        st.session_state.fairness_objective_getter = None
    ...
    def query_input_section():
        ...
        with right_half:
            with tab_objects[0]:
                with input_tab_objects[0]:
                    st.dataframe(st.session_state.dataset, height=300, width=1200)
        ...
        user_input = st.text_area(
            "Enter Query:",
            height=50,
            value=f"SELECT * FROM {db_name} WHERE ",
        )
        st.markdown("""<style>
            .stTextArea {
        ...
        with upload_col1:
            ...
            <style>
                ...
                [data-testid='stFileUploader'] section + div {
                    ...
                    """, unsafe_allow_html=True)
            st.session_state.selected_database = st.selectbox("Select Dataset or Upload CSV:", list(DATASETS.keys()),
                                                              index=1)
            st.markdown("""<style>
                .stSelectbox {
        ...
        if not st.session_state.got_input:
            ...
            if cont_button_1_placeholder.button("Continue", key="cont_query", use_container_width=True):
                ...
                if original_query:
                    ...
                    sql_engine = SQLEngineAdvanced(dfs)
                    st.session_state.original_output_dataset, _ = sql_engine.execute(original_query)
    ...
    if st.session_state.got_input:
        with right_half:
            with tab_objects[0]:
                ...
                with input_tab_objects[0]:
                    st.dataframe(st.session_state.input_dataset, height=300, width=1200)
                with input_tab_objects[1]:
                    st.dataframe(st.session_state.original_output_dataset, height=300, width=1200)
        ...
        with left_half:
            ...
            if "agent" in st.session_state.params:
                st.text_area("Query to refine:", height=50, value=st.session_state.params["query"], key="query_to_refine2")
                st.markdown("""<style>
                    .stTextArea {
    ...
    if st.session_state.show_constraints:
        with left_half:
            if "agent" in st.session_state.params:
                ...
                if "user_fairness" not in st.session_state and not st.session_state.toggle_constraints:

                    st.markdown("""
                            <div style="font-size: 17px; color: black; display:inline;">
                                Provide a brief natural language description of the task's constraints in the box below, to be generated into 
                                code by the system. 
                            </div>
                            """,
                                unsafe_allow_html=True)

                    if st.button("Alternatively, Press here to provide a python function that evaluates your constraints.", key="toggle_constraints_button"):
                    ...
                    with tab_objects[1]:
                        ...
                        st.markdown(
                            ...
                            <div style="font-size: 17px; color: black;">
                                Insert your Code below. Your code should include the threshold logic. 
                            </div>
                            """,
                            unsafe_allow_html=True
                        )
                        ...
                        if st.button("Alternatively, Press here to provide a natural language description for your constraints.", key="toggle_constraints_button2"):
                            st.session_state.toggle_constraints = not st.session_state.toggle_constraints
                            st.rerun()

                        st.markdown("")
                        st.markdown("")
                        fairness_objective_getter = st.text_area("", value=ORIG_VALUE, height=300)
                        # Toggle the state
                        if st.button("Accept Code", key="accept_code"):
                            st.session_state.fairness_objective_getter = fairness_objective_getter
                        change_button_shape()

            elif "user_fairness" not in st.session_state:
                with right_half:
                        ...
                        with right_half:
                            ...
                            with tab_objects[1]:
                                ...
                                with spinner_placeholder, st.spinner("Generating Constraint Satisfaction Deviation Objective..."):
                                    ...
                                    if MOCK:
                                        time.sleep(1)
                                        fairness_objective_getter = mock_ups.FAIRNESS_EVALUATION_MOCK
                                    else:
                                        agent = st.session_state.params["agent"]
                                        fairness_objective_getter = agent.generate_fairness_objective(function_description)
                                    st.session_state.fairness_objective_getter = fairness_objective_getter
                                    st.rerun()
    if st.session_state.fairness_objective_getter:
        if "user_fairness" not in st.session_state:
            with right_half:
                if st.session_state.show_constraints:
                    if "fairness_objective_getter_updated" not in st.session_state:
                        st.session_state.fairness_objective_getter_updated = st.session_state.fairness_objective_getter
                    html(switch_tab_js(1), height=0)
                    with tab_objects[1]:
                        constraints_logic()
            try:
                with left_half:
                    ...
                    if cont_button_2_placeholder.button("Continue", key="cont_fairness", use_container_width=True):
                        st.session_state.user_fairness = st.session_state.fairness_objective_getter
            ...
            with right_half:
                ...
                with tab_objects[1]:
                    with st.container(height=500):
                        st.code(st.session_state.fairness_objective_getter, language="python",
                                line_numbers=True,
                                wrap_lines=True)
            with left_half:
                try:
                    ...
                    pass
                # TODO - add the text above instead before the text_area
                st.markdown("""
                        ...
                        """,
                            unsafe_allow_html=True)
                st.markdown("""
                        <div style="font-size: 16px; color: blue; display:inline; text-decoration: underline;">
                            Alternatively, Press here to provide a python function that evaluates your constraints.
                        </div>
                        """,
                            unsafe_allow_html=True)
    ...
    # TODO - Do the same for the Refinement Objective
            ...
            if st.session_state.toggle_refinement:
                ...
                with left_half:
                    ...
                    with ref_col1:
                        ...
                        if st.session_state.refinement_type == 'Query':
                    ...
                    with ref_col2:
                        st.markdown(" ")
                        st.markdown(" ")
                        if st.button("Press here if you'd like to provide specific details for the refinement objective", key="toggle_specifics_button"):
                            st.session_state.toggle_specifics = not st.session_state.toggle_specifics
                        change_button_shape(button_index=0, margin_top=-3)
                    ...
                    if st.session_state.toggle_specifics:
                        with right_half:
                            ...
                            with tab_objects[2]:
                                ...
                                st.markdown(" ")
                                st.markdown(" ")
                                place_holder_txt = st.empty()
                                place_holder_button_spec = st.empty()
                                st.session_state.specifics = place_holder_txt.text_area(
                                    "Provide specifics for the refinement objective:", height=150)
                                if place_holder_button_spec.button("Generate Refinement Distance Objective using your provided specifics",
                                                                        key="generate_refinement_specifics"):
                                    if len(st.session_state.specifics) > 0:
                                        place_holder_txt.empty()
                                        place_holder_button_spec.empty()
                                        with spinner_placeholder, st.spinner(
                                                f"Generating specified {st.session_state.refinement_type} Refinement Objective..."):
                                            if MOCK:
                                                time.sleep(1)
                                                refinement_objective_getter = mock_ups.REFINEMENT_QUERY_SPEC_MOCK
                                            else:
                                                agent = st.session_state.params["agent"]
                                                refinement_objective_getter = agent.edit_refinement_distance_objective(
                                                    st.session_state.refinement_type, st.session_state.specifics)
                                            st.session_state.refinement_objective_getter = refinement_objective_getter
                                            st.rerun()
                                html("""<script>
                                    const buttons = window.parent.document.querySelectorAll(".stButton > button");
        ...
        with left_half:
            ...
            with ref_col1:
                st.radio(
                    ...
                    ('Query', 'Result'),
                    horizontal=True,
                    index=0 if st.session_state.refinement_type == 'Query' else 1)

            with ref_col2:
                st.markdown("""
                        <div style="font-size: 16px; color: blue; display:inline; text-decoration: underline; position: relative; top: 35px;">
                            Press here if you'd like to provide specific details for the refinement objective.
                        </div>
                        """,
                            unsafe_allow_html=True)
            st.markdown(
                "<div style='font-size: 14px;'>Select refinement-constraints weights:</div>",
                unsafe_allow_html=True)
            ...
            st.session_state.params["epsilon"] = float(epsilon / 100)
    ...
    if st.session_state.show_output:
        with right_half:
            ...
            exec(st.session_state.user_fairness)
            ...
            if "output_df" not in st.session_state:
                ...
                st.session_state.refinements_count = chat_writer.refinements_counter
            ...
            with tab_objects[4]:
                st.session_state.chat_writer.render_log()
                if st.button("Reset and run another query refinement problem", type="primary", key="reset2", use_container_width=True):
                    st.session_state.clear()
                    st.rerun()
                    html(switch_tab_js(0))
...
def output_logic():
    ...
    st.markdown(" ")
    st.markdown(
        f"<div style='position: relative; top: -10px; font-size: 17px;'><b> Constraints Score: </b>{st.session_state.constraint_score:.2f}</div>",
        unsafe_allow_html=True)
    st.markdown(
        f"<div style='position: relative; top: -10px; font-size: 17px;'><b> Refinement Score: </b>{st.session_state.refinement_score:.2f}</div>",
        unsafe_allow_html=True)
    st.markdown(
        "<div style='font-size: 20px;'><b>Constraints Score explanation:</b></div>",
        unsafe_allow_html=True)
...
def constraints_logic():
    ...
    if not st.session_state.edit_mode:
        with placeholder.container(height=500):
            st.code(st.session_state.fairness_objective_getter_updated, language="python", line_numbers=True,
                    wrap_lines=True)
        placeholder_button = st.empty()
        st.session_state.fairness_objective_getter = st.session_state.fairness_objective_getter_updated
        if placeholder_button.button("Edit Code", key="edit_constraints", use_container_width=True):
            ...
            if placeholder_button.button("Accept Code", key="accept_constraints", use_container_width=True):
                st.session_state.edit_mode = False
                placeholder.code(st.session_state.fairness_objective_getter_updated, language="python",
                                 line_numbers=True,
                                 wrap_lines=True)
                st.session_state.fairness_objective_getter = st.session_state.fairness_objective_getter_updated
        # html("""
